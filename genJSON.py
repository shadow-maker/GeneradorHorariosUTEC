	...
	if i["Frecuencia"] == "Semana General":
		...
		if i["Cod. Curso"] in cursos:
			if i["Sec"] in cursos[i["Cod. Curso"]]["secciones"]:
				cursos[i["Cod. Curso"]]["secciones"][i["Sec"]].append(newItem)
			else:
				cursos[i["Cod. Curso"]]["secciones"][i["Sec"]] = [newItem]
		else:
			cursos[i["Cod. Curso"]] = {"nombre": i["Nombre Curso"], "secciones": {i["Sec"]: [newItem]}}

for curso in cursos.items():
	newSecs = {}
	for sec in curso[1]["secciones"].items(): # Loop secciones en un curso
		if len([i for i in sec[1] if int(i["sesion"].split(".")[-1]) not in [0, 1]]) > 0:
			secGen = [i for i in sec[1] if int(i["sesion"].split(".")[-1]) == 0]

			for i in range(max([int(i["sesion"].split(".")[-1]) for i in sec[1]])):
				newSecs[sec[0] + "." + str(i + 1).zfill(2)] = secGen.copy()

			for i in [j for j in sec[1] if int(j["sesion"].split(".")[-1]) != 0]: # Loop sesiones en una seccion
				newKey = sec[0] + "." + str(i["sesion"].split(".")[-1]).zfill(2)
				newSecs[newKey].append(i)
		else:
			newSecs[sec[0]] = sec[1]

	# Editar seccion
	if len([int(i["sesion"].split(".")[-1]) for i in sec[1] if int(i["sesion"].split(".")[-1]) not in [0, 1]]) > 0:
		cursos[curso[0]]["secciones"] = newSecs.copy()
